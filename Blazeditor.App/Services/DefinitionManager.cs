using Blazeditor.App.Components.Pages;
...
namespace Blazeditor.App.Services
{
    public class DefinitionManager
    {
        ...
        public List<Tile> AddTilePaletteToArea(Area area, string filename, Blazeditor.App.Models.Size? baseCellSize = null)
        {
            if (area == null) throw new ArgumentNullException(nameof(area));
        ...
        public List<Tile> ExtractTilesFromImage(string filename, Blazeditor.App.Models.Size? baseCellSize)
        {
            if (string.IsNullOrWhiteSpace(filename)) throw new ArgumentException("Filename cannot be null or empty.", nameof(filename));
            var cellSize = baseCellSize ?? new Blazeditor.App.Models.Size { Height = 64, Width = 64 }; // Use default value if null
            ...
            using (var image = Image.Load<Rgba32>(imagePath))
            {
                foreach (var tileElem in tiles.EnumerateArray())
                {
                    ...
                    using (var ms = new MemoryStream())
                    {
                        ...
                        result.Add(new Tile(name, base64Url, new Blazeditor.App.Models.Size(w, h)));
                    }
                }
            }
            return result;
